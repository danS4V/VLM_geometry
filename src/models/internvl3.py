...
class InternVL3Model(Model):
  '''Class to store Intern VL 3 model, processor and vocab.
  HUGGINGFACE VERSIONS ONLY.
  '''

  def __init__(self,model_id = "OpenGVLab/InternVL3_5-8B-hf",**kwargs) -> None:
    self.__dict__.update(kwargs)
    self.model_id = model_id
    self.model, self.processor, self.vocab = self._get_intern(model_id)

  def _get_intern(self,model_id):
