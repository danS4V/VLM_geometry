...
from probes.fitters import FitIndepProbeNoTest, FitCorSProbeNoTest, FitJointProbeNoTest
...
def main(cfg: DictConfig) -> None:
  ...
  elif cfg.probe.type == 'ColorOrShape':
    if cfg.select_colshapes is not None:
      cslist = cfg.select_colshapes
    else:
      cslist = cfg.dataset.COLORS.copy()
      cslist.extend(cfg.dataset.SHAPES.copy())
    for prop in cslist: ## prop is either a color or a shape
      print("\033[92m",prop,"\033[0m")
      if prop in cfg.dataset.COLORS:
        proptype = 'color'
      elif prop in cfg.dataset.SHAPES:
        proptype = 'shape'
      else:
        raise KeyError(f'{prop} property not recognized')

      probe, singletraintest = FitCorSProbeNoTest(proptype,prop,metapd,outputs,probelabel,cfg)
      torch.save(probe.state_dict(),os.path.join(probeoutputdir,f'{prop}_{cfg.probe.versionlabel}.nn'))
      del probe
      traintest[prop]=singletraintest
      with open(traintestpath,'wb') as f:
        pickle.dump(traintest,f)
        #sometimes i may want to ctrl-c training, but keep info on what has already be done;
        #so i overwrite it every time instead then just at the end
